...
export default function AdminPage() {
  ...
  const handleBan = async (businessId: string, isBanned: boolean) => {
    if (!confirm(`Are you sure you want to ${isBanned ? 'unban' : 'ban'} this business?`)) {
      return;
    }
      ...
      <main className="flex-1 bg-gray-50 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            ...
            <p className="text-gray-600">Manage businesses and moderate reports</p>
          </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200">
                      ...
                      <th className="text-center py-3 px-4 font-semibold text-gray-700">Stats</th>
                  ...
                  <tbody>
                    {businesses.map((business) => (
                      <tr key={business.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="py-4 px-4">
                          <p className="text-sm text-gray-900">{business.whatsappNumber}</p>
                          <a
                        ...
                        <td className="py-4 px-4">
                          <p className="text-sm text-gray-900">{business.user.name}</p>
                          <p className="text-sm text-gray-600">{business.user.email}</p>
                        </td>
                        <td className="py-4 px-4 text-center">
                          <span className={`text-2xl font-bold ${
                            business.trustScore >= 80 ? 'text-green-600' :
                            business.trustScore >= 60 ? 'text-yellow-600' :
                            business.trustScore >= 40 ? 'text-orange-600' :
                            'text-red-600'
                          }`}>
                            {business.trustScore}
                          </span>
                        ...
                        <td className="py-4 px-4">
                          <div className="flex flex-col gap-2">
                            <button
                              onClick={() => handleVerify(business.id, business.isVerified)}
                            ...
                            <button
                              onClick={() => handleBan(business.id, business.isBanned)}
                              className={`px-3 py-1 rounded text-sm font-semibold transition-colors ${
                                business.isBanned
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="bg-white rounded p-3 mb-2">
                          <p className="font-semibold text-red-800 mb-1">{report.reason}</p>
