...
export default function TrustBadge({ score, isVerified, isBanned }: TrustBadgeProps) {
  if (isBanned) {
    return (
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className={`inline-flex items-center gap-2 px-4 py-2 ${bgColor} border-2 rounded-lg`}>
      ...
      {isVerified && (
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-100 border-2 border-blue-500 rounded-lg">
          ...
          <p className="text-sm font-semibold text-blue-800">Verified Business</p>
        </div>
      )}
    </div>
  );
